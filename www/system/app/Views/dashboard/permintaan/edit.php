...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 pt-3">
    <?= form_open_multipart('/permintaan/update/' . $permintaan['id']); ?>
    <?= csrf_field(); ?>
        ...
        <div class="form-floating mb-2">
            <select class="form-select rounded-3 <?= (validation_show_error('menu')) ? 'is-invalid' : ''; ?>" id="menu" name="menu" aria-label="menu">
                ...
                <?php foreach ($menu as $selectmenu) : ?>
                    <option value="<?= $selectmenu['id_menu']; ?>" <?= (old('menu')) ? ((old('menu') == $selectmenu['id_menu']) ? 'selected' : '') : (($permintaan['id_menu'] == $selectmenu['id_menu']) ? 'selected' : ''); ?>><?= $selectmenu['tanggal'] . ' - ' . $selectmenu['nama_menu'] . ' - ' . $selectmenu['nama_petugas']; ?></option>
                <?php endforeach; ?>
            </select>
