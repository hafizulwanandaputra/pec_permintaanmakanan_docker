        ...
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable rounded-3">
            <div class="modal-content bg-body shadow-lg transparent-blur">
                ...
                <div class="modal-footer justify-content-end pt-2 pb-2" style="border-top: 1px solid var(--bs-border-color-translucent);">
                    <button class="btn btn-primary btn-sm bg-gradient rounded-3" target="_blank" id="printeticket"><i class="fa-solid fa-print"></i> Cetak E-tiket</button>
                </div>
            </div>
        </div>
    </div>
...
- below function makes the datatable to export all records when "server side processing" is on */
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            $.ajax({
                url: '<?= base_url('/permintaan/permintaan') ?>/' + id,
                success: function(response) {
                    $('#demandModalLabel').text('Edit Petugas Gizi');
                    $('#demandId').val(response.id);
                    $('#id_menu_lama').val(response.id_menu);
        ...
        $('#demandForm').submit(function(e) {
            e.preventDefault();
            var url = $('#demandId').val() ? '<?= base_url('/permintaan/update') ?>' : '<?= base_url('/permintaan/create') ?>';
